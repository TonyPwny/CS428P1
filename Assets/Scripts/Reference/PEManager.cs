...
public class PEManager : MonoBehaviour
{
    ...
    public static Dictionary<GameObject, PEAgent> agentsObjs = new Dictionary<GameObject, PEAgent>();
    public Material pursuer;

    private static List<PEAgent> agents = new List<PEAgent>();
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < agentCount; i++)
        {
            ...
            {
                agent.tag = "Pursuer";
                agent.GetComponent<Renderer>().material = pursuer;
            ...
            var agentScript = agent.GetComponent<PEAgent>();
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out user_hit))
        {
            if (Input.GetButton("Fire1"))
            {
                destination = user_hit.point;
    ...
    {
        var agent = obj.GetComponent<PEAgent>();
